    ...
    <section className='w-full bg-white py-10 sm:py-16 md:py-24'>
      <div className='container mx-auto px-4 sm:px-6'>
        ...
        <div className='text-center mb-8 sm:mb-12 md:mb-16'>
          ...
          <p className='text-black text-base sm:text-lg md:text-[22px] font-light leading-relaxed mt-2 max-w-2xl mx-auto'>
          ...
          {industries.map((industry, index) => {
            return (
              <div key={index} className='flex flex-col items-center text-center space-y-3 sm:space-y-[16px]'>
                ...
                {/* Title */}
                <h3 className='mb-1 sm:mb-3 text-lg sm:text-xl md:text-[26px] font-medium'>{industry.title}</h3>

                {/* Description */}
                <p className='text-sm sm:text-base md:text-[21px] font-thin text-black/70'>{industry.description}</p>
              </div>
            );
          })}
        </div>
